...
#define Matrix_C std::vector<std::vector<uint8_t>>
...
void HopfieldNetwork::load_sample(const std::vector<double> &sample, bool check_sample_count)
{
    ...
    for (int i = 0; i < neuron_count; i++)
    {
        if (sample[i] < 0 || sample[i] > 1)
            throw SampleFormatException("Sample values must be 0 to 1");
...
void HopfieldNetwork::load_sample(const Matrix_C &sample, bool check_sample_count)
{
    // Load sample from char matrix (matrix values must be 0 to 255)
    Matrix_D new_m;
    resize_matrix(new_m, sample.size(), sample[0].size());
    for (int i = 0; i < sample.size(); i++){
        for (int j = 0; j < sample[0].size(); j++){
            new_m[i][j] = 1 - sample[i][j] / 255;
        }
    }
    load_sample(new_m, check_sample_count);
}

// void HopfieldNetwork::load_sample(const Image& img, bool check_sample_count){
...
std::vector<double> HopfieldNetwork::compare(const Matrix_C &img_matrix)
{
    // Clear noise from img by matrix
    Matrix_D new_m;
    resize_matrix(new_m, img_matrix.size(), img_matrix[0].size());
    for (int i = 0; i < img_matrix.size(); i++){
        for (int j = 0; j < img_matrix[0].size(); j++){
            new_m[i][j] = img_matrix[i][j] / 255;
        }
    }
    return compare(new_m);
}
